...
| **空间记忆** | 每次放置自动记录 (物体/坐标/时间)，支持上下文查询 |
...
| 🔍 空间查询 | `"刚才放的是什么"` `"把刚刚放的东西拿回来"` | 上下文记忆 |
| ❓ 帮助 | `"帮助"` `"你能做什么"` | 功能说明 |

> 💡 30 条关键词规则覆盖 13 个类别 + 空间记忆上下文查询，断网也能用。
...
每次 pick-and-place 操作自动记录：物体、颜色、抓取坐标、放置坐标、时间戳。

| 能力 | 示例 | 说明 |
|------|------|------|
| 📍 上下文抓取 | `"把刚刚放的东西拿回来"` | 自动查找最后放置的物体 |
| 🔍 位置查询 | `"刚才放的是什么"` | 口头回复物体位置 |
| 💾 自动记录 | 每次操作后 | 存于 `memory/spatial_memory.json` |
| 🌐 API 查询 | `GET /api/spatial-memory` | 获取完整历史 |

```json
// memory/spatial_memory.json 示例
{
  "records": [{
    "time_str": "12:30:15",
    "object_desc": "红色杯子",
    "color": "red", "class_name": "cup",
    "pick_x": 50, "pick_y": 100,
    "drop_x": 0,  "drop_y": 120
  }]
}
```

> 💡 空间记忆使机械臂理解 "刚刚"、"上次"、"那个" 等上下文指代。
...
│   ├── spatial_memory.py      #   空间记忆 (物体坐标→自动记录)
