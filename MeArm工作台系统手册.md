...
| 波特率 | 115200 |
| 关节限位 | base/left/right: 30-150°, claw: 10-73° |
| 回零位 | base=90, left=90, right=90, claw=73 |
...
- NumPy < 2.0 (MediaPipe 兼容性要求)
- Windows 11 (支持 DSHOW 摄像头后端)
...
# 2. ⚠️ 杀干净所有残留 Python 进程（否则串口会冲突）
pkill -9 -f python; sleep 3

# 3. 纯软件测试模式（不连硬件，全功能可用）
PYTHONIOENCODING=utf-8 python workbench_server.py

# 4. 连 Arduino 安全模式（USB 供电，单舵机顺序运动）
PYTHONIOENCODING=utf-8 python workbench_server.py --port COM3

# 5. 外接电源全速模式
PYTHONIOENCODING=utf-8 python workbench_server.py --port COM3 --unsafe

# 6. 使用手机 IP 摄像头
PYTHONIOENCODING=utf-8 python workbench_server.py --ip-cam http://192.168.x.x:8080/video
```
...
| `--deepseek-key` | DeepSeek API 密钥 | 内置默认 |
